...
class LineFollower(object):
	def __init__(self):
		...
		self.image = rospy.Subscriber("/usb_cam/image_raw",Image,self.camera_callback)
	...
	def camera_callback(self,data):
		...
		try:
			...
			higher_lower_red = np.array([160,100,100])
			higher_upper_red = np.array([179,255,255])


			lower_red_hue_range = (lower_lower_red,lower_upper_red)
			higher_red_hue_range = (higher_lower_red,higher_upper_red)


			#lower_blue = np.array([80,110,80])
			#upper_blue = np.array([240,235,255])

			
			#lower_blue = np.array([95,110,80])
			#upper_blue = np.array([115,255,250])


			#mask = cv2.inRange(hsv, lower_red, upper_red)

			mask_high = cv2.inRange(hsv, lower_lower_red, lower_upper_red)
			mask_low = cv2.inRange(hsv, higher_lower_red, higher_upper_red)

			mask = cv2.addWeighted(mask_high, 1, mask_low, 1, 0.0)
