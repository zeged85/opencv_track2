...
class WebcamVideoStream:
    ...
    def proccess(self):
        ...
        #mask
        #red
        #lower_red = np.array([0,83,72])
        #upper_red = np.array([10,255,255])            
        #mask1 = cv2.inRange(hsv, lower_red, upper_red)
        #lower_red = np.array([167,83,72])
        #upper_red = np.array([179,255,255])            
        #mask2 = cv2.inRange(hsv, lower_red, upper_red)
        #mask = cv2.bitwise_or(mask1,mask2)
        #green
        lower = np.array([80,24,75])
        upper = np.array([95,255,255])
        #Blue
        #lower = np.array([94,124,110])
...
class LineFollower(object):
        ...
        def follow(self, vs):
                ...
                if not (vs.target_found and vs.object_location != None):
                        self.drive(0,0)
                        return
                pid = self.camera_pid
                pid.kp = -0.001
                pid.kd = 0.004
                target_value = 0.0
                current_value = vs.object_location
...
def main():
        ...
        #line_follower_object.startCapture(vs) 

        while not (line_follower_object.recieved_scan):
        ...
        while not rospy.is_shutdown():
                ...
                try:
                        ...
                        line_follower_object.follow(vs)
