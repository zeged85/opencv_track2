...
class LineFollower(object):
        ...
        def camera_callback(self,data):
                ...
                try:
                        ...
                        descentre = -140
                        rows_to_watch = 200
                        crop_image = data[(height)/2+descentre:(height)/2+(descentre+rows_to_watch)][1:width]
                        #crop_image = cv_image
                        
                        #blur
                        #blur = cv2.GaussianBlur(crop_image, (15,15), 0)
                        #blur = cv2.bilateralFilter( crop_image, 5, 75 ,75)               
                        blur = self.bridge_object.imgmsg_to_cv2(crop_image, desired_encoding="bgr8")
           
                        #hsv
                        hsv = cv2.cvtColor(blur,cv2.COLOR_BGR2HSV)

                        #mask
                        lower = np.array([58,80,40])
...
def init(setting):
        ...
        image_info = rospy.wait_for_message("/usb_cam/camera_info", CameraInfo)
        print ('got image')
